...
static SDL_Surface *sdlwindowsurf = NULL;
...
long getvalidmodelist (validmodetype **davalidmodelist)
{
	...
	if (!validmodecnt) {
		...
		{
			const SDL_PixelFormatDetails *details = SDL_GetPixelFormatDetails(modes[i]->format);
			gvmladd(modes[i]->w, modes[i]->h, details->bits_per_pixel);
		}
...
void updatepalette (long start, long danum)
{
	...
	int i,j;
	SDL_Palette *sdlsurfpal;

	if (!sdlsurf) return;

	sdlsurfpal = SDL_CreateSurfacePalette(sdlsurf);

	for (i=0,j=danum; j>0; i++,j--) {
		colors[i].r = pal[start+i].peRed;
		colors[i].g = pal[start+i].peGreen;
		colors[i].b = pal[start+i].peBlue;
		colors[i].a = SDL_ALPHA_OPAQUE;
	}
	SDL_SetPaletteColors(sdlsurfpal, colors, start, danum);
}
...
long startdirectdraw(long *vidplc, long *dabpl, long *daxres, long *dayres)
{
	...
	if (!SDL_LockSurface(sdlsurf)) return 0;
...
void nextpage()
{
	if (!sdlsurf) return;
	if (surflocked) stopdirectdraw();
	if (!SDL_BlitSurface(sdlsurf, NULL, sdlwindowsurf, NULL))
		fprintf(stderr, "SDL_BlitSurface failed: %s\n", SDL_GetError());
}

long clearscreen(long fillcolor)
{
	return SDL_FillSurfaceRect(sdlsurf, NULL, fillcolor) == 0;
}
...
long initdirectdraw(long daxres, long dayres, long dacolbits)
{
	...
	sdlwindow = SDL_CreateWindow(prognam, daxres, dayres, 0);
	sdlwindowsurf = SDL_GetWindowSurface(sdlwindow);
	SDL_SetWindowSurfaceVSync(sdlwindow, 1);

	switch (dacolbits)
	{
		case 8: fmt = SDL_PIXELFORMAT_INDEX8; break;
		case 16: fmt = SDL_PIXELFORMAT_RGB565; break;
		case 32: fmt = SDL_PIXELFORMAT_RGBX32; break;
		default: fprintf(stderr, "unsupported pixel bits %d\n", dacolbits); return 0;
	}

	sdlsurf = SDL_CreateSurface(daxres, dayres, fmt);
	if (!sdlsurf || !sdlwindow || !sdlwindowsurf) {
		fputs("video init failed!",stderr);
		return 0;
	}

	details = SDL_GetPixelFormatDetails(sdlsurf->format);

	// FIXME: should I?
	xres = sdlsurf->w;
	yres = sdlsurf->h;
	colbits = details->bits_per_pixel;
	fullscreen = SDL_GetWindowFlags(sdlwindow) & SDL_WINDOW_FULLSCREEN;

	memset(&curvidmodeinfo, 0, sizeof(validmodetype));
	curvidmodeinfo.x = sdlsurf->w;
	curvidmodeinfo.y = sdlsurf->h;
	curvidmodeinfo.c = details->bits_per_pixel;
	// FIXME: is this used anywhere in ken's code? --erysdren
#if 0
	curvidmodeinfo.r0 = details->Rshift;
	curvidmodeinfo.rn = 8 - details->Rloss;
	curvidmodeinfo.g0 = details->Gshift;
	curvidmodeinfo.gn = 8 - details->Gloss;
	curvidmodeinfo.b0 = details->Bshift;
	curvidmodeinfo.bn = 8 - details->Bloss;
	curvidmodeinfo.a0 = details->Ashift;
	curvidmodeinfo.an = 8 - details->Aloss;
#endif
...
//--------------------------------------------------------------------------------------------------
void sdlmixcallback(void *userdata, SDL_AudioStream *stream, int additional_amount, int total_amount)
{
	long amt;
	...
	amt = min(streambuf->buflen - streambuf->writecur, total_amount);
	SDL_PutAudioStreamData(stream, &((Uint8*)streambuf->buf)[ streambuf->writecur ], amt);
	streambuf->writecur += amt;
	if (streambuf->writecur == streambuf->buflen) streambuf->writecur = 0;
	total_amount -= amt;
	if (total_amount > 0) {
		SDL_PutAudioStreamData(stream, &((Uint8*)streambuf->buf)[ streambuf->writecur ], amt);
		streambuf->writecur += total_amount;
	}
	
	// mix in the umixer buffers
}

static SDL_AudioStream *sdlaudiostream = NULL;
...
static int kensoundinit (int samprate, int numchannels, int bytespersamp)
{
	...
	audwant.format = bytespersamp == 1 ? SDL_AUDIO_U8 : SDL_AUDIO_S16LE;
	audwant.channels = numchannels;
	// audwant.samples = MINBREATHREND;
	// audwant.callback = sdlmixcallback;

	if (!(sdlaudiostream = SDL_OpenAudioDeviceStream(SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK, &audwant, sdlmixcallback, NULL)))
		return -1;
...
void setacquire (long mouse, long kbd)
{
	if ((mouse || kbd) != mouse_acquire) {
		if (mouse) mouse_acquire = SDL_SetWindowMouseGrab(sdlwindow, true);
		if (kbd) kbd_acquire = SDL_SetWindowKeyboardGrab(sdlwindow, true);
	...
	while (SDL_PollEvent(&ev)) {
		switch (ev.type) {
			...
			case SDL_EVENT_WINDOW_FOCUS_GAINED:
				shkeystatus = 0;
				gbstatus = 0;
				ActiveApp = ev.type == SDL_EVENT_WINDOW_FOCUS_GAINED;
				if (mouse_acquire) {
					if (ActiveApp) {
						SDL_SetWindowKeyboardGrab(sdlwindow, true);
						SDL_SetWindowMouseGrab(sdlwindow, true);
			...
			case SDL_EVENT_WINDOW_RESIZED:
				ActiveApp = 1;
				initdirectdraw(ev.window.data1, ev.window.data2, colbits);
				break;
#endif
			case SDL_EVENT_KEY_DOWN:
				sc = keytranslation[ev.key.key];
				...
				if (ev.key.key != 0 && (ev.key.key & 0xff80) == 0) {
					if (keybufw2 != keybufr) //stick ASCII code in last FIFO value
						keybuf[(keybufw2-1)&(KEYBUFSIZ-1)] |= (ev.key.key & 0x7f);
				}
				break;
...
int main(int argc, char **argv)
{
	...
	if (!SDL_Init(sdlinitflags)) { fputs("Failure initialising SDL.",stderr); return -1; }
	atexit(evildie);   // In case we exit() somewhere
